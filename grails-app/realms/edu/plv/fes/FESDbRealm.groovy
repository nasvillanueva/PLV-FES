...
class FESDbRealm {
    ...
    def authenticate(authToken) {
        ...
        // exception.
        def user = FESUser.findByUsernameAndActive(username,true)
        if (!user) {
            throw new UnknownAccountException("No active account found for user [${username}]")
        }
        ...
        // in the database.
        def account = new SimpleAccount(username, user.passwordHash, "FESDbRealm")
    ...
    def hasRole(principal, roleName) {
        def roles = FESUser.withCriteria {
            roles {
                eq("name", roleName)
            }
    ...
    def isPermitted(principal, requiredPermission) {
        ...
        def user = FESUser.findByUsername(principal)
        ...
        def results = FESUser.executeQuery("select distinct p from FESUser as user join user.roles as role join role.permissions as p where user.username = '$principal'")
